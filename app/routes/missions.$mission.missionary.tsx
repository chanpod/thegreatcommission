...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
	...
	const missionsWithMissionaries = await db
		.select()
		.from(missions)
	...
	const availableMissionaries = await db
		.select()
		.from(users)
		.where(
			and(
...
const AddMissionary = () => {
	...
	return (
		<div>
			...
			<div>
				{loaderData?.missionaries.length === 0 ? (
					<span>No missionaries to add right now</span>
				) : null}
				<List>
					{loaderData?.missionaries?.map(
						(missionary: typeof users.$inferSelect) => {
							return (
								<MissionaryListItem
									editing
									key={missionary.id}
