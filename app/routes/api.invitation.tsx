...
export const action = async ({ request, params }: ActionArgs) => {
	...
	if (invitationType === InvitationTypes.Organization) {
		const invitationRequest: Partial<typeof organizationMembershipRequest> = {
			type: invitationType,
			requestingChurchOrganizationId: formData.get(
				"requestingChurchOrganizationId",
			) as string,
			parentOrganizationId: formData.get("parentOrganizationId") as string,
		};

		const existingInvitation =
			await prismaClient.organizationMemberShipRequest.findFirst({
				where: {
					AND: {
						status: "pending",
						requestingChurchOrganizationId:
							invitationRequest.requestingChurchOrganizationId as string,
						parentOrganizationId:
							invitationRequest.parentOrganizationId as string,
					},
				},
			});
