...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
	const missionResponse = await db
		.select()
		.from(events)
		.where(eq(events.id, params.mission as string))
		.innerJoin(
			churchOrganization,
			eq(events.churchOrganizationId, churchOrganization.id),
		)
		.then((res) => res[0]);
...
const MissionaryPage = () => {
	...
	function deleteChurch() {
		deleteFetcher.submit(
			{},
			{ method: "delete", action: `/missions/${loaderData.mission?.id}` },
		);
	}
			...
			<div className="flex">
				<div className="flex-1">
					<div className="flex items-center justify-start">
						...
						<div className="flex flex-col justify-center items-center">
							<div className="text-green-700 text-3xl flex items-center">
								...
								<CurrencyFormatter
									skipDollarSign
									value={loaderData.mission?.investment ?? 0}
								/>
							</div>
					...
					<div className="text-sm text-gray-500 items-center flex">
						...
						{format(new Date(loaderData.mission?.startDate), "MM-dd-yyyy")}{" "}
						until {getEndTime()}
					</div>
				...
				{isLoggedIn && (
					<DropdownMenu>
						...
						<DropdownMenuContent align="end">
							<DropdownMenuItem
								onClick={() =>
									navigate(`/missions/${loaderData.mission?.id}/edit`)
								}
							>
								Edit
							</DropdownMenuItem>
							<DropdownMenuItem
								onClick={() =>
									navigate(`/missions/${loaderData.mission.id}/missionary`)
								}
							>
								Edit Missionaries
							</DropdownMenuItem>
			...
			<Dialog open={showDialog} onOpenChange={setShowDialog}>
				<DialogContent>
					...
					<DialogFooter>
						...
						<Button variant="destructive" onClick={deleteChurch}>
							Delete
						</Button>
					</DialogFooter>
				</DialogContent>
