...
import { Link, useSearchParams, useSubmit, useLoaderData, useParams, useNavigate, Outlet } from 'react-router'
...
export const loader = async ({ params }: Route.LoaderArgs) => {
    ...
    const now = new Date()
    const upcomingEvents = calendarEvents.filter(event => new Date(event.startDate) >= now)
    const previousEvents = calendarEvents.filter(event => new Date(event.startDate) < now)
...
export default function EventsLayout() {
    const { events, upcomingEvents, previousEvents } = useLoaderData<typeof loader>()
    ...
    const handleSelectEvent = (event: CalendarEvent) => {
        navigate(`/churches/${organization}/events/${event.id}/edit?${searchParams.toString()}`)
    }
