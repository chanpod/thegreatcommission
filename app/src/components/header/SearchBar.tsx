...
export type ISearchEntityTypes =
	| typeof churchOrganization
	| typeof missionaries
	| typeof missions;
...
const SearchBar = (props: Props) => {
	...
	function onSelected(
		...
		if (props.onSelected) {
			switch (entityType) {
				...
				case SearchEntityType.Missionary:
					setSearch(
						(selected as typeof missionaries).firstName +
							" " +
							(selected as typeof missionaries).lastName,
					);
					break;
				case SearchEntityType.Mission:
	...
	return (
		<div
			ref={ref}
			className={`dark pt-1 pb-1 relative z-20 text-white text-sm rounded-lg w-11/12 block`}
		>
			{headerStyle ? (
				<input
								...
								<div className="relative flex-col gap-8 lg:grid-cols-2 border-gray-200">
									{props.showHeaders !== false ? (
										<span className="lg:text-lg flex items-center">
											Missionaries{" "}
											{missionaries.length === 0 && search.length >= 2 && (
												<NoResults />
											)}
										</span>
									) : null}
							...
							{props.loadMissions !== false && (
								<div className="relative flex-col gap-8 lg:grid-cols-2 border-gray-200">
									{props.showHeaders !== false ? (
										<span className="lg:text-lg flex items-center">
											Missions{" "}
											{missions.length === 0 && search.length >= 2 && (
												<NoResults />
											)}
										</span>
									) : null}
							...
							{props.loadChurches !== false && (
								<div className="relative  flex-col gap-8  lg:grid-cols-2 border-gray-200">
									{props.showHeaders !== false ? (
										<span className="lg:text-lg flex items-center">
											Organizations{" "}
											{churches.length === 0 && search.length >= 2 && (
												<NoResults />
											)}
										</span>
									) : null}

									<ChurchOrganizationList
										churches={churches}
										onSelected={onSelected}
									/>
								</div>
